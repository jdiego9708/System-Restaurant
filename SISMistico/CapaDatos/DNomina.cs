    ...
    public class DNomina
    {
        ...
        public async Task<(string rpta, DataTable dtNominaEmpleado)> BuscarNomina(string tipo_busqueda, string texto_busqueda)
        {
            ...
            try
            {
                ...
                consulta.Append("SELECT * FROM Nomina_empleado nom ");
                consulta.Append("INNER JOIN Empleados em ON nom.Id_empleado = em.Id_empleado ");

                if (tipo_busqueda.Equals("ID NOMINA"))
                {
                    consulta.Append("WHERE nom.Id_nomina_empleado = ").Append(texto_busqueda + " ");
                }
                else if (tipo_busqueda.Equals("ESTADO"))
                {
                    consulta.Append("WHERE nom.Estado_nomina = ").Append(texto_busqueda + " ");
                }
                else if (tipo_busqueda.Equals("FECHA"))
                {
                    consulta.Append("WHERE nom.Fecha_nomina = ").Append(texto_busqueda + " ");
                }

                consulta.Append("ORDER BY nom.Id_nomina_empleado DESC ");
